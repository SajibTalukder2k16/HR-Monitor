...
class findFaceGetPulse(object):
    ...
    def __init__(self, bpm_limits=[], data_spike_limit=250,
        ...
        self.total_bpm = 0  # riad
        self.count = 0 # riad
        self.t = [] # riad
        self.new_bpm = []
        self.latest_data = []  # riad
    ...
    def run(self, cam):
        ...
        if self.find_faces:
            ...
            cv2.putText(
                self.frame_out, "Press 'S' to lock face and begin",
                       (10, 25), cv2.FONT_HERSHEY_COMPLEX, .65, col,1,cv2.LINE_AA)
            cv2.putText(self.frame_out, "Press 'Esc' to quit",
                       (10, 50), cv2.FONT_HERSHEY_COMPLEX, .65, col,1,cv2.LINE_AA)
        ...
        cv2.putText(self.frame_out, "Press 'F' to save data",
                   (10, 75), cv2.FONT_HERSHEY_COMPLEX, .65, col,1,cv2.LINE_AA)
        cv2.putText(self.frame_out, "Press 'Esc' to quit",
                   (10, 100), cv2.FONT_HERSHEY_COMPLEX, .65, col,1,cv2.LINE_AA)
        ...
        if L > 10:
            ...
            if idx:
                ...
                if pruned.size:
                    ...
                    '''  updated code to estimate bpm in 10 sec '''
